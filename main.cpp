...
void say(node* tree, int& node_count, int& leaf_count) {
    if (tree != nullptr) {
        node_count++; 
        if (tree->leaf)
            leaf_count++; 
        for (int i = 0; i < tree->key_count + 1; i++)
            say(tree->children[i], node_count, leaf_count); 
    }
}
int main() {
    node* tree = nullptr;
    ...
    int node_count = 0;
    int leaf_count = 0;
    say(tree, node_count, leaf_count);
